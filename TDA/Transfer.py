...
class LightningTransferModel(pl.LightningModule):
    def __init__(self, num_class, base_net='resnet152', use_bottleneck=True, bottleneck_width=256, learning_rate=1e-3):
        ...
        self.model = TransferModel(25, base_net, use_bottleneck, bottleneck_width)

        state_dict = torch.load('/home/amirkh/Python/Main/adapted-model/final_model_25.pt')
        ...
        if use_bottleneck:
            ...
            feature_dim = self.model.base_network.output_num()
        
        self.model.classifier_layer = nn.Linear(feature_dim, num_class)
        
        for param in self.model.classifier_layer.parameters():
...
if __name__ == "__main__":
    ...
    train_dataset = Target(csv_file, data_dir, split='train', split_column='1', transform=tf_train, dataset_type='F1', subsample=0.02)
    val_dataset = Target(csv_file, data_dir, split='val', split_column='1', transform=tf_test, dataset_type='F1')
    test_dataset = Target(csv_file, data_dir, split='test', split_column='1', transform=tf_test, dataset_type='F1')
